...
export default function ManagerUserUpdatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleToggle = async (field: "present" | "food", value: boolean) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.success(`${field === "present" ? "Attendance" : "Meals"} updated`);
      }
    } catch {
  ...
  if (!participant) {
    return (
      <div className="text-center py-24">
        ...
        <Link href="/manager/participants" className="text-[#FF9900] hover:underline text-sm font-semibold">
          ← Back to Search
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {/* Profile card */}
      <div className="bg-[#1A222D] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
        <div className="h-1 w-full bg-gradient-to-r from-[#0073BB] via-[#00A9E0] to-[#0073BB]/30" />
        ...
        <div className="p-8 flex flex-col sm:flex-row gap-8 items-start">
          ...
          <div className="shrink-0 flex flex-col items-center gap-3">
            ...
            <div className="flex gap-2">
              <Link
                ...
                target="_blank"
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-white/5 text-white/50 hover:text-white hover:bg-white/10 transition w-full justify-center"
              >
                <ArrowSquareOut weight="bold" className="w-3.5 h-3.5" /> View Live Ticket
              </Link>
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            <p className="font-mono text-[#00A9E0] text-xs font-bold tracking-widest mb-1">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start gap-3 text-sm">
                ...
                <span className="text-white/60">AWS SCD 2026 Delegate</span>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-5 flex-wrap">
              ...
              >
                {participant.food ? "✓ Meal Validated" : "Meal Pending"}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Update controls */}
      <div className="bg-[#1A222D] border border-white/5 rounded-2xl p-6 shadow-xl">
        <h2 className="text-white font-bold text-lg mb-5">Desk Operations</h2>
        <div className="space-y-3">
          {/* Present toggle */}
          <label
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-white font-semibold">Mark as Checked In</p>
                <p className="text-white/30 text-xs mt-0.5">
                  {participant.present ? "Participant is active" : "Verify delegate ID before enabling"}
                </p>
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-4">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                  participant.food ? "bg-[#FF9900]/20" : "bg-white/5"
                }`}
              >
                {participant.food ? (
                  <CheckCircle weight="fill" className="w-5 h-5 text-[#FF9900]" />
                ) : (
                  <XCircle weight="fill" className="w-5 h-5 text-white/20" />
                )}
              </div>
              <div>
                <p className="text-white font-semibold">Mark Meal Dispensed</p>
                <p className="text-white/30 text-xs mt-0.5">
                  {participant.food ? "Meal has been distributed" : "Toggle to record meal delivery"}
                </p>
              </div>
            </div>
            <div className="relative shrink-0">
              <div
                ...
                className={`w-12 h-6 rounded-full transition-all duration-300 cursor-pointer relative ${
                  participant.food ? "bg-[#FF9900]" : "bg-white/10"
                }`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all duration-300 ${
