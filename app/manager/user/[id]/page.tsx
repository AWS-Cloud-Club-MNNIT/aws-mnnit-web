...
export default function ManagerUserUpdatePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
    ...
    const loadParticipant = async () => {
      try {
        ...
        if (res.ok) {
          ...
          try {
            const url = `${window.location.origin}/ticket/${data.participant.participantId}`
  ...
  const handleToggle = async (field: "present" | "food", value: boolean) => {
    ...
    try {
      const res = await fetch(`/api/user/${id}`, {
        ...
        body: JSON.stringify({ present: field === "present" ? value : participant.present, food: field === "food" ? value : participant.food }),
      })
      const data = await res.json()
      if (!res.ok) { toast.error(data.error || "Failed to update"); setParticipant(previous) }
      else toast.success(`${field === "present" ? "Attendance" : "Meals"} updated`)
    } catch { toast.error("Network error"); setParticipant(previous) }
  ...
  const handleVerificationUpdate = async (status: "pending" | "verified" | "rejected") => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { toast.error(data.error || "Failed to update"); setParticipant(previous) }
      else { setParticipant(data.participant); toast.success(`Status set to "${status}"`) }
    } catch { toast.error("Network error"); setParticipant(previous) }
    finally { setUpdating(null) }
  }
  ...
  if (!participant) {
    return (
      <div className="max-w-2xl mx-auto text-center py-32 bg-[#131920]/80 border border-white/5 rounded-3xl mt-12 shadow-2xl">
        ...
        <p className="text-white/40 mb-8">No delegate record for ID: <span className="font-mono text-[#FF9900] font-bold bg-[#FF9900]/10 px-3 py-1 rounded-lg inline-block mt-2">{id}</span></p>
        <Link href="/manager/participants" className="inline-flex items-center bg-white text-black font-bold px-8 py-4 rounded-xl hover:scale-105 transition-all">
          Return to Registry
        </Link>
      </div>
    )
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <Link href="/manager/participants"
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#FF9900]">Desk verification module</span>
          <button
        ...
        <div className="p-10 flex flex-col md:flex-row gap-12 items-center md:items-start relative z-10">
          ...
          <div className="flex-1 min-w-0 text-center md:text-left pt-2">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-8">
              ...
              {participant.verifiedBy && <span className="text-white/30 text-xs">by {participant.verifiedBy}</span>}
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="bg-[#131920]/80 border border-white/5 rounded-[2rem] p-8 shadow-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <button onClick={() => handleVerificationUpdate("verified")} disabled={!!updating || participant.verificationStatus === "verified"}
            className={`flex flex-col items-center gap-2 p-5 rounded-2xl border-2 font-bold transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed ${participant.verificationStatus === "verified" ? "bg-green-500/20 border-green-500/50 text-green-400 shadow-[0_0_20px_rgba(74,222,128,0.2)]" : "bg-green-500/5 border-green-500/20 text-green-400/70 hover:bg-green-500/15 hover:border-green-500/40"}`}>
          ...
          <button onClick={() => handleVerificationUpdate("pending")} disabled={!!updating || participant.verificationStatus === "pending"}
            className={`flex flex-col items-center gap-2 p-5 rounded-2xl border-2 font-bold transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed ${participant.verificationStatus === "pending" ? "bg-amber-500/20 border-amber-500/50 text-amber-400 shadow-[0_0_20px_rgba(251,191,36,0.2)]" : "bg-amber-500/5 border-amber-500/20 text-amber-400/70 hover:bg-amber-500/15 hover:border-amber-500/40"}`}>
          ...
          <button onClick={() => handleVerificationUpdate("rejected")} disabled={!!updating || participant.verificationStatus === "rejected"}
            className={`flex flex-col items-center gap-2 p-5 rounded-2xl border-2 font-bold transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed ${participant.verificationStatus === "rejected" ? "bg-red-500/20 border-red-500/50 text-red-400 shadow-[0_0_20px_rgba(239,68,68,0.2)]" : "bg-red-500/5 border-red-500/20 text-red-400/70 hover:bg-red-500/15 hover:border-red-500/40"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Label className={`flex flex-col p-8 rounded-[2rem] cursor-pointer transition-all duration-300 relative overflow-hidden border-2 ${participant.present ? "bg-green-500/10 border-green-500/30 hover:bg-green-500/20" : "bg-[#131920]/80 border-white/5 hover:border-white/10 backdrop-blur-xl"} ${updating === "present" ? "opacity-50 pointer-events-none" : ""}`}>
          ...
          <p className="text-white/40 text-sm">{participant.present ? "Participant is inside the event perimeter." : "Authorize entry for this delegate."}</p>
        </Label>

        <Label className={`flex flex-col p-8 rounded-[2rem] cursor-pointer transition-all duration-300 relative overflow-hidden border-2 ${participant.food ? "bg-[#FF9900]/10 border-[#FF9900]/30 hover:bg-[#FF9900]/20" : "bg-[#131920]/80 border-white/5 hover:border-white/10 backdrop-blur-xl"} ${updating === "food" ? "opacity-50 pointer-events-none" : ""}`}>
          ...
          <p className="text-white/40 text-sm">{participant.food ? "Meal protocol complete." : "Issue meal rights to this delegate."}</p>
        </Label>
      </div>
    </div>
  )
